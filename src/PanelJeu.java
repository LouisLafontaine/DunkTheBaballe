			...
			if(balle.moving){
				...
				for(Obstacle o : obstacles){
					if(balle.hasCollided(o)){
						...
						animatedItems.add(new Animated("Animation/explosion2.png",(int)balle.x-32,
								(int)balle.y-64,1,8,0, 16,1,true));
						break;
					}
				}
				checkSolveWin();
...
//======================================================================
	...
	public void drawRectangleFromMouse(Graphics g) {
		...
		if(clickX>lastClickX && clickY>lastClickY){
			g.fillRect(lastClickX, lastClickY, multipleOfThick(clickX-lastClickX),
					multipleOfThick(clickY-lastClickY));

		}else if(clickX<lastClickX && clickY>lastClickY) {
			g.fillRect(multipleOfThick(clickX),
					lastClickY,
					multipleOfThick(lastClickX-multipleOfThick(clickX)),
					multipleOfThick(clickY-lastClickY));

		}else if(clickX>lastClickX && clickY<lastClickY) {
			g.fillRect(lastClickX,
					multipleOfThick(clickY),
					multipleOfThick(clickX-lastClickX),
					multipleOfThick(lastClickY-multipleOfThick(clickY)));
	...
	private void addNewObstacleFromMouse() {
		if(clickX>lastClickX && clickY>lastClickY){
			obstacles.add(new Obstacle(lastClickX,
					lastClickY,
					multipleOfThick(clickX-lastClickX),
					multipleOfThick(clickY-lastClickY)));
			...
			obstacles.add(new Obstacle(multipleOfThick(clickX),
					multipleOfThick(clickY),
					multipleOfThick(lastClickX-multipleOfThick(clickX)),
					multipleOfThick(lastClickY-multipleOfThick(clickY))));
	}
